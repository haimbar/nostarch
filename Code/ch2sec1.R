...
boxplot(y, axes=F, border="purple", col="grey66")
axis(2)
dev.off()
...
boxplot(y ~ g, axes=F, border=c("navyblue","darkgreen"), col=c("wheat1", "thistle1"), xlab="", ylim=c(min(y), max(y)*1.1))
...
hist(y[g==1], breaks=20, xlim=c(min(y), max(y)), col="#00008090", border="grey66", add=T)
dev.off()

pdf("images/chapter_2/scatter.pdf", width=4, height=4)
plot(x, y, pch=19, col="navyblue", cex=0.3, axes=F)
axis(1)
axis(2)
dev.off()
...
plot(x, y, pch=19, col=cols, cex=0.3, axes=F)
axis(1)
axis(2)
