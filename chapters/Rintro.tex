...
directory (which is denoted by `\slash' on Mac and Linux, or \why{Windows start
  with a drive name.} `Drive:\textbackslash' on Windows). The path corresponds to the hierarchical organization of folders (directories) on your computer.
...
is \why{These values are not correct. They are the means of other
  distributions. The code file needs to be separated.} \inlnR{```cat(mean(x))```}, the trimmed mean is  \inlnR{```cat(mean(x, trim=0.1))```}, and the median is \inlnR{```cat(median(x))```}.
...
The `whiskers', which are the \why{They are not visible; the adjusted minimum
  and maximum are not dashed lines.} dashed grey lines, are constructed by adding 1.5$\cdot$IQR to each side of the box. If the result is smaller than the minimum value (or greater than the maximum), then the whisker only extends to the minimum (maximum). Points within the range between the two whiskers are not plotted individually, since their distribution is summarized succinctly by the box-and-whiskers plot. Points outside the range between the two whiskers are considered `outliers', or extreme values, and are shown explicitly. 
...
so we say that the distribution is skewed to the right. {\color{blue}\why{Not
    clear, remove it?} The median does not depend on the scale of the data. It simply represents where half the data lies.}
...
use $\mu=2.5$ and \why{this $\sigma$ value does not seem to match the figure.} $\sigma^2=0.25$. The orange curve shows the function $\phi$, by using the \code{dnorm()} function (to obtain the density function of $x$).
...
the kurtosis \textcolor{red}{will we use the kurtosis later? If so, we should
  add a ref.} \why{It doesn't seem that we will for this book, but we can keep
  this mark here in case we do.}.
