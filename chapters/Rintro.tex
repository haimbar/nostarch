...
example, the following code save \code{plkerHand} in a CSV file and read it as
...
We can define our own functions in R. We define a function for finding the
summation and we see that it produce the same result as the base \code{sum}
function. 
...
so we say that the distribution is skewed to the right. {\color{blue}\why{
    Help check this:} The median does not depend on observations in the tails
  of the data (small and large observations in the data). It simply represents
  where the data can be split half. Thus the median is a more robust measure of
  the central tendency of the data, and it is more appropriate than the mean for
screwed data such as incomes.}
