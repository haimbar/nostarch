...
To install \R go to the Comprehensive R Archive Network website: 
...
Environment and a History tab. Click on \textbf{History}. All your previous
input appears there. Try entering another calculation or command in the console.
...
to get the documentation for \lstinline{date()}, try the following:
...
\noindent In this case the output should be
 \likelst{\inlnR{```cat(class(courseNames))```}}, which means that this variable is
...
We can also get the number of elements in a vector by using the 
...
\likelst{\inlnR{```cat(paste(oddLT20))```}}.
...
The variable \lstinline{ones} contains the values: \likelst{\inlnR{```cat(paste(ones))```}},
and the sum is  \likelst{\inlnR{```cat(sum(ones))```}}. Their cumulative sum of
...
following: \likelst{\inlnR{```cat(concatvar)```}}.
...
\lstinline{sample()} function to do this:
...
example, the following code saves \lstinline{pokerHand} in a
 ...
 represents one observation, and for each observation consists of one or more
...
In this example, player 3 got two aces (clubs and diamonds), the queen of hearts,
...
sequence of commands a \emph{function}. Once we have a function, we don't
...
meaning that in every segment of length $\ell$ between 0 and 1, we expect to
have the same number of random points (in this case, $10,000/\ell$).
...
\likelst{\inlnR{cat('From sum:', sum(simData), '. From mysum:', mysum(simData))}}.
...
The code should produce a \emph{histogram}, a graph in which all the observations
...
\lstinline{FALSE} (\emph{Boolean}) values. First, we draw \lstinline{ntrials}
...
it allows us to run \emph{vectorized code}. In one line, we generated 200 random
...
\likelst{\inlnR{```cat(sum(runif(ntrials) < 0.5))```}}. Statistical inference is based
...
say, 100 times. One way is to use \emph{loops}, like in the following example:
...
distribution, called the \emph{binomial distribution}. The mathematical notation
...
the following, which uses the \lstinline{rbinom()} function:
...
than $x_{0.5}$ and half are less than or equal to $x_{0.5}$. 
...
In this example with $n=$ \likelst{\inlnR{```cat(n)```}} random numbers we
get that the mean
is \likelst{\inlnR{```cat(mean(x))```}}, the trimmed mean is
\likelst{\inlnR{```cat(mean(x, trim=0.1))```}}, and the median is
\likelst{\likelst{\inlnR{```cat(median(x))```}}}.
...
\textit{box-and-whisker plot} (or simply, boxplot). This is a very simple representation
...
\emph{first quartile}, a point, $x_{0.25}$, such that 25 percent of the values are less
...
quantiles (also called percentiles). The $q$-th quantile is a number $t$, such that
...
\lstinline{summary()} function. It gives the minimum, maximum, the 25th, 50th,
...
each bin. Another possibility is to use the \lstinline{cut()} function,
which divides a range of numbers into discrete ranges and creates categories
in a \emph{factor} variable.
...
to find the \emph{mode} of the data (the most frequent value):
\showCode{R}{Code/ch1sec3b.R}[1][2]
The hotel with the largest number of guests is
\inlnR{```cat(levels(hotelrooms)[which.max(hoteltbl)])```} with
\inlnR{```cat(max(hoteltbl))```} guests, and
\inlnR{```cat(levels(autorental)[which.max(autotbl)])```}
is rented to \inlnR{```cat(max(autotbl))```} people.
