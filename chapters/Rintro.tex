...
Notice the top-right part of the Rstudio window. You should see both an \textbf{Environment} and a \textbf{History} tab. Click on History. All your previous input appears there. Try entering another calculation or command in the console. For example, try entering the following
...
Try the following: \lstinline{cumsum(cumsum(ones))} (but first try to think what the result will be).

There are also many functions to handle text. The \lstinline{paste()} function attaches two strings of characters together. The default is to concatenate strings with a space as a separator, but we can use the collapse option (also called an `argument' of the function) to use a different separator. For example,
...
Let's `deal' five cards to each player in a game of poker. We will use the \lstinline{sample()} function.
