...
\item Event: a subset of the sample space is called an event; an event occurs
...
probability of event $E$ is \begin{equation}
...
2. The \emph{unconditional} probability of getting an even number in a roll of
a die is 1/2, but because of the information that provided to us, we conclude
that the \emph{conditional} probability is 1/3.  Using mathematical notation,
...
such that $P(A)$ is greater than zero. Then \begin{equation}\label{eq:bayes}
...
$A>C$}\label{Intransitive-Dice}} Real numbers are transitive, meaning that if
