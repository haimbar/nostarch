...
\hypertarget{Randomworld}{The world is random and probability is a tool to quantify the randomness. It's the mathematical basis for all statistical reasoning. It helps us to determine how likely it's that we'll win a lottery, or the chances that our town will be hit by an earthquake in the next month.
...
When there are two events $A$ and $B$ under consideration, knowing that $A$ has occurred may change the probability for $B$ to occur. For example, suppose that someone rolls a die behind a curtain and tells us only that the number is less than or equal to 3. What is the probability that it's an even number? Given that we got 1, 2, or 3, the only possibility that it's even is if the number is 2. The \emph{unconditional} probability of getting an even number in a roll of a die is 1/2, but because of the information that provided to us, we conclude that the \emph{conditional} probability is 1/3. 
...
One of the most important results in probability and statistics looks deceivingly simple, but has deep consequences, as we'll see in some of the subsequent examples. it's called Bayes' Rule, named after the reverend Thomas Bayes who discovered it in the 18th century. Consider two events, $A$ and $B$, such that $P(A)$ is greater than zero. Then
...
people has the same birthday as yours, there are only 22 pairs to consider for the event to occur
