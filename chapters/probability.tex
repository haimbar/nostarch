...
\begin{itemize}
\item Experiment\index{Experiment}: a situation in which the outcome occurs randomly, e.g.,
  flipping a coin, rolling a dice, and picking a card from a deck.
\item Sample Space\index{Sample Space}: the set of all possible outcomes in an experiment, e.g.,
...
an event to occur. If we assume (for now) that the sample space is finite and
...
game, and $B$ is the events that she attends the weekly ballet}
...
\begin{exercise}
  Adapt the previous code by changing $N$ and $n$, to check the
...
winner, i.e., Tom would win at least three out of the five games.  \wingding{4}
...
complicated. Some people didn't believe the result answer until they were shown some
...
made 3 of the free throws. If at the end of game three the coach want to pick
...
blood, i.e., the probability that he is the burglar?
...
again pulling the trigger as
\inlnR{```cat(prob.revolver[1])```}[probability_prob.revolver1], and the
approximate probability for keep shooting as
\inlnR{```cat(prob.revolver[2])```}[probability_prob.revolver2]. 
respectively. 
...
\begin{exercise}
Use simulation to find the probability that some prisoners
...
in \wingding{9} simulates the procedure of smartly opening 50 drawers. We uses
...
\code{i} (\wingding{11}). The \code{for} loop in \wingding{12} the following 49
