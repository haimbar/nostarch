...
\begin{itemize}
\item \index{Experiment} Experiment: a situation in which the outcome occurs randomly, e.g.,
  flipping a coin, rolling a dice, and picking a card from a deck.
\item \index{Sample Space} Sample Space: the set of all possible outcomes in an experiment, e.g.,
\{head, tail\}, \{1, 2, ..., 6\}, and \{all card faces\}.
\item \index{Event} Event: a subset of the sample space is called an event; an event occurs
...
\emph{Probability} is a number between zero and one which tells us how likely
...
2. The \index{Probability!unconditional} \emph{unconditional} probability of getting an even number in a roll of
a die is 1/2, but because of the information that provided to us, we conclude
that the \index{Probability!conditional} \emph{conditional} probability is 1/3.  Using mathematical notation,
...
-- in this case we say that the two events are \emph{independent}, and we have
$$P(B\mid A) = P(B)\,.$$ \index{Event!independence}
...
\index{Event!mutually exclusive}
...
\begin{example}[Two Children]\index{Two children problem}
...
subsequent examples. It's called Bayes' Rule, named after the reverend Thomas
...
\index{Simpson's paradox!UC Berkeley admissions} 
...
The \emph{base-rate fallacy} is a common source of misconceptions due to
