...
By now you've learned to explore data, reason about probability,
...
artificial intelligence, or AI.\index{artificial intelligence} Large language
...
wrong, and how the judgment you've built up over the previous
...
This chapter contains no R code, and that's deliberate, for a few
...
In this chapter, the word AI mainly means generative AI tools,
especially large language models that can produce text, code, and
other content.\index{generative AI}\index{large language model}
Different tools behave differently from one another, and even the
...
the principles in this chapter---what AI is good at, what it's bad
...
\section{Why Learn Data Science in the AI Era}

Learning data science still matters because AI can carry out steps without
understanding whether those steps make sense. AI can write a command, fit a
model, or describe a graph, but it doesn't reliably know which question is
worth asking or what the variables in your dataset really measure. It can make
the work faster without making the reasoning easier.

Think of an AI tool as a very fast assistant in the data-analysis game. You
still have to decide what game you're playing. Are you describing a pattern,
predicting an outcome, comparing two groups, or asking whether one change
caused another? You also need to know whether your data contain the information
needed for that goal. Learning data science gives you a larger set of
questions and methods to consider, instead of limiting you to whatever task
you happen to think of first.

You also have to explain the situation to your assistant. Before asking for
code, decide who the population is, what the sample represents, how each
variable was measured, and what outcome would answer the question. Tell the AI
about the context and constraints that matter. A polished prompt can't replace
knowledge of the subject or of how the data were collected.

Finally, you have to check the proposed move before accepting it. Suppose you
ask an AI tool whether one group of people has a higher average than another.
It might immediately choose a test, run the calculation, and report a small
$p$-value. But it might not notice that the groups were formed by a biased
sampling process, that the variable was measured differently in the two
groups, or that the question is about a causal effect rather than an
association. The calculation can be correct while the conclusion is still
misleading.

The ideas in this book give you the checks you need: examine the data, inspect
the assumptions, verify the calculations, and distinguish an association from
a causal claim. AI can help you explore more possibilities and complete the
routine steps more quickly. It can't take responsibility for deciding what
the evidence supports. The more fluent the tool becomes, the more useful your
own understanding is when you decide whether its answer deserves to be used.

\section{What AI Can Do for You}

Used as a collaborator rather than an oracle, AI can
speed up almost every stage of a data analysis project.

\begin{itemize}
\item \textbf{Writing clean, documented code.} AI tools are often good at
...
\item \textbf{Producing visualizations.} AI can quickly generate a
  first draft of a plot, including plots you haven't made before, and
  suggest alternative ways to display the same data.
\item \textbf{Getting a second opinion.} Because AI is trained on an
  enormous amount of code and writing from many practitioners, asking
  it for another possible approach can be a little like asking a large
  group of experienced analysts how they would approach a problem.
\end{itemize}
...
\begin{itemize}
\item \textbf{Hallucinated output.}\index{hallucination} AI tools sometimes
  invent function names, arguments, citations, or numerical results
  that look plausible but don't exist or are simply wrong. A
  hallucinated result can be stated just as confidently as a correct
  one, so it's not always obvious which is which.
\item \textbf{Unreproducible workflows.}\index{reproducibility} An AI
  assistant isn't a substitute for version control or a documented
...
\item \textbf{False confidence.} AI tends to state its answers in a
  ...
  correct. Hallucinated output is a problem with what the tool says;
  false confidence is a problem with how easily we trust it. This is
  sometimes called \emph{automation
  bias}\index{bias!automation bias}: the tendency to trust a
  computer-generated answer more than it deserves, simply because it's
  delivered so confidently. Nothing about the tone of a response
  tells you how reliable it is.
\item \textbf{Data leakage.}\index{data leakage} Pasting a
...
The most effective way to use AI in a data analysis project isn't to
hand it a dataset and ask for ``the answer.'' It's to bring the
...
and understand why they're missing, look for a plausible source of
...
Chapter~\ref{ch:regression} showed that linear regression isn't
always the right model for the outcome at hand. When you ask an AI
tool to recommend a statistical method, don't simply accept the
...
It's just as important to ask AI to flag a limitation you've seen
throughout this book: association isn't causation, and this matters
most when the data didn't come from a designed experiment. The NYC
...
these results describe an observational dataset and shouldn't be read
...
the conclusions you're entitled to draw depends on that distinction.

\section{General Principles for Working with AI}

The tools available today won't be the tools available in a few
years, but a few working habits should remain useful regardless of
which AI you use.

\begin{itemize}
\item Treat AI as a fast, capable, but overconfident collaborator,
  not as a source of ground truth.
\item Start with the question and the data, not with a method name. Explain
  the population, sample, variables, and outcome to the AI, and ask it to say
  what information is still missing before it recommends an analysis.
\item Ask the AI to explain its reasoning and assumptions. Then check whether
  those assumptions fit the way the dataset was collected and the variables
  being analyzed.
\item Verify anything that matters: rerun the code yourself, check a cited
  number against its source, and confirm that a function or argument actually
  exists before relying on it.
\item Keep a reproducible record of the work. Save the data description, the
  prompts that mattered, the code you actually used, and the final results in
  files or a version-control system.
\item Never share data covered by a privacy rule, an institutional agreement,
  or a confidentiality obligation with a general-purpose AI tool unless you
  know exactly where that data goes.
\end{itemize}
...
practitioners. We also discussed its real risks: hallucinated output,
unreproducible workflows, false confidence, and data
...
analysis as AI continues to change. AI can make technical steps faster,
but the statistical judgment needed to choose a question, understand a
dataset, and decide what the evidence supports is still yours to learn.
