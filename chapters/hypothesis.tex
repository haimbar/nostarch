...
\emph{null hypothesis}\index{null hypothesis},
$H_0$, which represents
the default position or baseline assumption, and the
\emph{alternative hypothesis}\index{alternative hypothesis},
...
(a \emph{Type~I  error}\index{Type~I error}), or fail to 
reject $H_0$ when $H_1$ is true (a \emph{Type~II error}\index{Type~II
...
decision errors. A \emph{Type~I error}\index{type I error}
...
The probability of making this error is called the \emph{significance
  level}\index{significance}, denoted by $\alpha$. A smaller $\alpha$ 
reduces this risk but may make it harder to detect real effects.
A \emph{Type~II error}\index{type II error} arises when we fail to
...
called the \emph{power}\index{power} of the test, masures the test's
...
In hypothesis testing, a \emph{test statistic}\index{test statistic} 
...
falls into a designated \emph{rejection region}\index{rejection region}. 
...
\emph{hypergeometric distribution}\index{distribution!hypergeometric}, 
