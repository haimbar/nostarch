...
(asymptotically) normal. The expression in~(\ref{eq:1}) is complicated to
...
We see that the least squares estimates with the blood pressure data are $\hat{\beta}_0$=\inlnR{```cat(b[1])```}[sbp-beta0] and $\hat{\beta}_1$=\inlnR{```cat(b[2])```}[sbp-beta1].

Now let's look at the motivating example of body fat measurement. 
