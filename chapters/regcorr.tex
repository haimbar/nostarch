\hypertarget{ch:regression}{%
  \chapter{Correlation and Linear Regression: When Two Variables Dance}
  \label{ch:regression}}
...
\why{More topics to add:}
\subsection{Outliers}
\subsection{model diagnostics, R squared, etc.}
\subsection{Prediction Interval}
When we use a linear regression model to predict the value of $y$ for a given
\subsection{Linear models is not always linear}
Linearity means linear in parameters
\subsection{Be careful of extrapolation}
Extrapolation is the process of using a linear regression model to predict
values outside the range of the original data, which can be unreliable because
the linear trend may not hold true for those new values. While you can find a
predicted y-value by plugging an x-value outside the original range into the
model's equation, the results are often inaccurate and can lead to nonsensical
conclusions, especially as the prediction moves further from the original data.
