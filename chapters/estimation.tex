...
to construct estimates, compare different estimati
...
\emph{midrange} (the midpoint between the sample minimum and sample maximum).  
...
Figure~\ref{fig:est:violin-games} shows the same race in a more visual way for
...
$100(1-\alpha)\%$ of repeated samples.
...
\label{ex:ci-clt}
\runR{Code/est-ci-clt.R}{ci-clt}[run]
...
\runR{Code/est-ci-boot.R}{ci-boot}[run]
...
features of a population and how uncertainty affects these estimates.
...
different levels of bias, variability, and mean squared error. We
...
the observed data. Throughout the chapter, simulation played a central
role in understanding estimation and uncertainty.
...
\begin{exercise}
Use the Cauchy hide-and-seek code, but replace \lstinline{rcauchy} with
