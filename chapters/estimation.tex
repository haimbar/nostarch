...
to construct estimates, compare different estimation strategies, discover
the idea behind maximum likelihood estimation, and quantify uncertainty
using confidence intervals and resampling methods.
...
The location parameter $\mu$ of a normal distribution coincides with its mean and
...
three candidate estimators, or strategies to seek the true~$\mu$:
\begin{itemize}
\item Sample mean
\item Sample median
\item Midrange
...
underlying data generation model, which is unknown. There is, however,
an estimator which is the most efficient under quite general assumptions.
...
inferences. This leads to interval estimators. The two endpoints of an interval
