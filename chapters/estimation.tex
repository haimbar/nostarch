...
Their values are \inlnR{```cat(estimates[1])```},
\inlnR{```cat(estimates[2])```}, and \inlnR{```cat(estimates[3])```}, respectively.
...
Their values are \inlnR{```err[1]```},
\inlnR{```cat(err[2])```}, and \inlnR{```cat(err[3])```}, respectively.
...
The results are
\inlnR{```print(rowMeans(sim.t))```}[][vbox]
...
from $t$ distributions with other digrees of freedom. One can verify that as the
degrees of freedom increases (so the $t$ distribution gets closer to the normal
distribuition), the mean estimator eventually outperforms the median estimator.
\end{example}
...
The results are
\inlnR{```print(rowMeans(sim.u))```}[][vbox]
...
\subsection{Maximim Likelihood Estimator}
...
xselected, and the ball is red. Which jar was the one that was selected?
...
the unknown target parameter with certaiin probability. A
...
may be derived in some situations, but even in these situations, the asyptotic
...
\inlnR{```cat(medBoot$t0)```}, with 95\% confidence
interval \lstinline{medCI}, which is
$(\inlnR{```cat(medCI[1])```}, \inlnR{```cat(medCI[2])```})$.
The true value \inlnR{```cat(target)```} is nicely captured by the interval
from this sample.
...
The mean of the point estimate \lstinline{mean(sim_50[1])} from the
\inlnR{```cat(nrep)```} replicates is
\inlnR{```cat(mean(sim_50[1,]))```}, which is very close to the true target
\inlnR{```cat(target)```}.


The empirical coverate rate of the 95\% confidence interval from the
\inlnR{```cat(nrep)```} replicates is
\inlnR{```cat(mean(sim_50[2,] < target & target < sim_50[3,]))```},
which is reasonably close to the nominal level 95\%.


We repeate the process for a smaller sample size~20:
\showCode{R}{Code/est-ci-boot.R}[39][40]
The empirical coverate rate of the 95\% confidence interval from the
\inlnR{```cat(nrep)```} replicates is
\inlnR{```cat(mean(sim_20[2,] < target & target < sim_20[3,]))```},
