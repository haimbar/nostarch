...
\inlnR{```cat(mean(sim_50[1, ] < mu & sim_50[2,] > mu))```}[estsim50].
...
\inlnR{```cat(mean(sim_20[1, ] < mu & sim_20[2,] > mu))```}[estsim20].
...
\inlnR{```cat(mean(sim_50[1,]))```}[estmeansim50], which is very close to the true target
...
\inlnR{```cat(mean(sim_50[2,] < target & target < sim_50[3,]))```}[estmeansim50ci],
