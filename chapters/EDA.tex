...
\chapterartfile{images/static/mirrorhist.png}
\hypertarget{EDA}{%
...
tools. However, it is very important to always look at the data, and this means
creating plots and tables in order to understand it better. We call this process
``exploratory data analysis'', or EDA, and it is a critical step which should
never be skipped. The type of plot which we generate in each case depends on the
data, and usually there is more than one option to choose from. In this chapter
we cover several visualization methods, and in subsequent chapters we will see
additional ones.

It is useful to think of data as a table in which each row represents one
...
\item \textit{Quantitative}: a variable that can take numerical values. It
  ...
  \begin{itemize}
  \item \textit{Continuous}: a variable which can take any value within
  ...
  \item \textit{Discrete}: a variable which can take at most a countable number
...
\item \textit{Boolean}: Some variables are either \code{TRUE} or
  \code{FALSE}. In the coin-flipping simulation in Chapter~\ref{rintro} we
  generated a Boolean variable which was \code{TRUE} if a randomly generated
  number drawn uniformly between zero and one was greater than 0.5, and
  \code{FALSE} otherwise. Boolean variables are categorical, with two levels.
...
\item \textit{Vectors, matrices, and data frames}: Multiple values of the
  ...
  into a vector via the \code{c()} function or generated via the \code{rep()},
  \code{seq()}, or random generating functions such as
  \code{rnorm()}. Similarly, vectors of the \textit{same type} can be combined
...
Note that in the third line we create a variable called \code{g} in two steps:
first, we draw a random sample of the same length as that of the vector \code{x}
...
to a factor by using the function \code{factor()}. Use the \code{str()} function
to see the structure and levels of this variable. In line 4 we create the
variable we are interested in, \code{y}, through some formula which we will
discuss later. In line 6 we create a Boolean variable, \code{yL}, which is set
to TRUE if the variable \code{y} exceeds the threshold (25) and FALSE
otherwise. We then combine the vectors \code{x}, \code{g}, \code{y} and
\code{yL} into one data frame called \code{df1}.
Enter \code{str(df1)} to see the structure of \code{df1}. Note that the columns
...
For numerical variables like \code{x} and \code{y}, \R shows the mean and the
`five-number summary': the minimum, the maximum, and the 25th, 50th, and 75th
percentiles. For categorical variables such as \code{g} and \code{yL} it shows
the counts in each category. For \code{yL} it also shows that it is a special
...
\code{y} and its possible relationship with other variables. In real life, we
...
\code{y}. The simplest graphical representation is the boxplot which was
...
Note the usage of the options of the \code{boxplot} function, which are
optional, but improve the way the plot looks. The \code{axes=F} option means
...
can run the \code{colours()} function.
...
depicted graphically, we can see that the distribution of \code{y} is
approximately symmetric, and there are no outliers. However, we can't say much
more about how \code{y} is distributed.
...
of observations in each bin. This is done with the \code{hist()} function:
\showCode{R}{Code/ch2-03hist.R}[2][2]
\runR{Code/ch2-03hist.R}{ch2-03hist}

Again, notice the usage of the function's arguments. For example, \code{breaks}
determines the number of bins, which \code{col} and \code{border} determine the
color of the interior and edges of each bin. The \code{main=""} option is used
in order to remove the title of the plot. The result is shown in
Figure~\ref{histy}.
Try changing these values. What happens when you set \code{breaks} to be
smaller, or larger?
...
to see if our variable \code{y} is somehow related to any of the other
variables, which is what we will do next.

\subsection{Side by side boxplots}
In addition to \code{y}, we have a categorical variable called \code{g}. To see
if it is related to \code{y} we can show one boxplot for each level of the
factor \code{g}, which, in this case means two side-by-side boxplots. The syntax
...
means `show how \code{y} varies by \code{g}'. Again, we suppress the axes and
show only the vertical one. We also suppress the label of the x-axis using
\code{xlab=""}. For each level of the factor \code{g} we use a different
combination of border and interior colors. We set the vertical range of the plot
using \code{ylim=c(min(y), max(y)*1.1))}, so that it will have some extra space
at the top of the figure. We use this space to put the labels for each group
with the \code{text()} function.

The first argument of the \code{text()} function contains the x-coordinates of
the text. In this case we have two values: 1 and 2. Why not 0 and 1, which are
the actual levels of the factor \code{g}? This is because, within \R levels of
...
maximum value of \code{y} and the upper limit of the vertical axis, so that the
...
\code{cex=0.6} which sets the font size to be 60\% of the default.
...
Is there a difference between the two groups, as determined by \code{g}? It's
...
When we use the brackets, as in \code{y[g==0]}, we tell \R to apply the
histogram function only to the subset of the data for which \code{g} has the
value 1.

We use the \code{xlim} argument to make sure that the range of the horizontal
...
The second time we call \code{hist} we use the \code{add=TRUE} option, which
...
color "navyblue". If we run \code{col2rgb("navyblue")} we get the numbers 0,0,
and 128, which means that the mixture has 0 red, 0 green, and the intensity for
the blue is 128/255. We then run \code{rgb(t(col2rgb("navyblue"))/255)} and get
\#000080. Each pair of digits corresponds to the three base colors. Then, to add
transparency we can do \code{rgb(t(col2rgb("navyblue"))/255, alpha = 0.5)} add
...
Next, we want to see if the variable \code{y} is related to \code{x} in some
way. Since both are continuous variables, we can display the relationship
between them with a scatterplot. In the following code we use the \code{plot()}
function, which \R infers to mean a scatterplot of the first argument \code{x}
vs. the second, \code{y}, because both are continuous. The other arguments to
...
cyclic pattern. Second, in the first half of the x-range, \code{y} appears to
...
now) a line that represents the relationship between \code{x} and \code{y}, the
...
So far we saw that there is a complicated relationship between \code{x} and
\code{y}, and maybe some relationship between \code{g} and \code{y}. Can we put
...
First, we use the \code{ifelse()} function to set the color for each group. The
...
\code{g} is equal to 1, are drawn in navy blue, and all the others in orange.
...
cyclic pattern and a distinct change-point where the value of \code{y} increases
abruptly, and then continues to increase as \code{x} becomes larger.

We also added a horizontal reference line at \code{y=30} by using the
\code{abline()} function. We set its type to dashed, its color to red, and its
...
We can imagine a hypothetical situation where \code{y} is a temperature of an
...
\begin{enumerate}
\item Without the $x$ term: \code{y <- 10 * (as.numeric(g) - 1) + rnorm(length(x), mean=0, sd=3)} 
\item Without the cyclical term: \code{y <- 3 * (x > 2 * pi) * x +  10 *
    (as.numeric(g) - 1) + rnorm(length(x), mean=0, sd=3)} 
\item Without the group term: \code{y <- 3 * (x > 2 * pi) * x +  rnorm(length(x), mean=0, sd=3)} 
\item Without the random noise: \code{y <- 3 * (x > 2 * pi) * x + 4 * sin(3 * x) + 10 * (as.numeric(g) - 1)} 
\item With less noise: \code{y <- 3 * (x > 2 * pi) * x + 4 * sin(3 * x) + 10 * (as.numeric(g) - 1) + rnorm(length(x), mean=0, sd=1)} 
\item With more noise: \code{y <- 3 * (x > 2 * pi) * x + 4 * sin(3 * x) + 10 *( as.numeric(g) - 1) + rnorm(length(x), mean=0, sd=5)} 
...
We use the \code{rpois()} function to draw random numbers which are all
...
\code{cut()} function is used to create categories from continuous data. It can
then be summarized by using the \code{table()} function, which simply counts the
total number of observations in each category:
\inlnR{```print(table(xcat))```}[vbox]
This can be shown graphically by using the \code{barplot()} function as shown in
Figure~\ref{barplot} (or as a pie chart by using the \code{pie()} function.)
...
\$10,000). The levels for \code{x} correspond to 1. dropped out before
...
\item Finally, we live in an era of big data, and for that we sometimes need to
  ...
  We generate 100,000 points \code{(x,y)} and construct a scatterplot (in line 3, which
