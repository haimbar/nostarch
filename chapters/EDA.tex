...
factor. The
ordered is determined alphabetically by default, so 0 is represented as 1, and
1 as 2.
...
Using the  \lstinline{alpha} argument adds adds to hexadecimal digits to
...
vs. the second, \lstinline{y}, because both are continuous. The other arguments
...
\begin{listhead}
\item [Exercise~2.1] Try running the simulation code, but with the following
changes: \begin{enumerate}
...
integers (from the Poisson distribution). We then create categories by using
the
\lstinline{cut()} function is used to create \emph{bins} based on ranges of the
...
principal of maximizing the `information to ink ratio', and Albert Einstein
said
...
understanding the data, and to communicating 
results.
