...
and a parakeet was stolen. He calls the police, who find two things on the scene:
...
it's type $B-$. They test a blood sample from Teddy, and the test shows that
he has that blood type. They immediately arrest Teddy for the burglary.
What is the probability that Teddy is the burglar?

To answer that, we need a bit more information. Let's say that the test for
detecting if Teddy's blood type is $B-$ is very accurate: if Teddy's blood type
is really $B-$ the rest result always shows $B-$; if Teddy's blood type is not
$B-$, it correctly shows another blood type 99\% of the times. Due to this
test's high accuracy, the data collected is of good quality, so the prosecutor
claims that Teddy is the burglar with a probability of 0.95 (95\%). This seems
convincing because the test is wrong only 5\% of the times, but the probability
that Teddy actually has type $B-$ blood is not 0.95 given a positive result. Let's say that the pet store is in London. In the
U.K., 2\% of the population has a $B-$ blood type. Let's check if the
prosecutor's claim true, by simulation.
...
\wingding{4}. Among all the positive results (the police test Teddy's
blood type as $B-$), we count the frequency that the person really has $B-$
blood type \wingding{5}. This approximates the probability that Teddy really has
$B-$ blood, i.e., the probability that he is the burglar?
...
\textcolor{red}{more examples? 1. Sampling from a periodic function, 2. the x1*x2 function?}
...
Some biases are due to human tendencies. For example, 
\emph{observation time interval bias} may occur if we end the data
collection process too early, or worse---when it conveniently provides
us results support the desired conclusion.

This is a special case of \emph{confirmation bias}, which occurs when people
...
to events based on how easy it is to recall them. Exposure to news
items can lead to this type of bias. Topics that get much media
attention (like diagnoses of a syndrome or a disease) or disasters
tend to be on peoples' mind and it's common for people to overestimate
