...
\jy{We've used sample and population many times in earlier
  chapters. Should we say now a formal definition? Or define them
  somewhere earlier.}
...
\jy{This does not define bias}
is systematically off from the true value, and it doesn't improve when we
...
In the code, \wingding{1} fits a linear regression model of weight on height
using R's \code{lm()} function (we will cover \code{lm()} in detail in the next chapter;
for now, think of it as finding the best-fit line through the data);
\wingding{2} uses the model's residuals---the differences between each person's actual weight
and the fitted value---to build a sampling weight \code{w}; and
\jy{I feel this might more complicated than necessary. How about
  setting up a sample obtained from subjects who ...}
\wingding{3} uses \code{w} to decide the probability of including a person in the sample. The larger the value of \code{w}, the higher the
probability of including the person in the sample. The exact way we 
\jy{could we make the example easier without invoking the residuals?
  such as a sample selected from those at the gym?}
...
When the probability of drawing a subject into the sample depends on their
height, weight, or both, then the sample is no longer representative
of the population, and estimates of population features based
on it will be biased.
...
variance is called \code{var1} and is equal to
\inlnR{```cat(var1)```}[var1], which is very close to the
\jy{the two variances being so close could be misleading. Change seed?}
...
\end{example}
\jy{Styles: the example environment needs an ending indicator; the
  closing of the example could include the solution in addition to the problem?}
...
\jy{How about removing the pdf and dev.off lines from showcode?}
...
\jy{avoid starting a sentence with a symbol (line number)?}
\jy{highlight the implication of independently drawn: no true good
  book bad movie relationship.}
...
Running the above code should give the two figures in Figure~\ref{fig:ch7:berkson}
...
This type of selection bias problem is related to the idea of \emph{conditional probability}\index{\emph{conditional probability}},
...
However, a missing variable is not always easy to spot. To see why this matters,
let's look at another example. Phil is a realtor in a small college town. His
