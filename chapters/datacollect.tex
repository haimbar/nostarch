\chapter{Data Collection: Catching the Right Data in the Wild}\label{ch:data}
You can't make bricks without straw. In the previous chapters we built
tools---probabilities, law of large numbers, the central limit
theorem, and statistical inference---that let
us reason carefully from a sample to a population. All of those guarantees rest on
one crucial assumption: the sample must be \emph{representative} of the population.
This chapter examines what representativeness means, what can go wrong when a sample
is not representative, and how to recognize the most common pitfalls in data
collection.

\section{Representative Sample}\label{representative}
\subsection{Definitions of Key Terms}\label{keyterms}
Although we have used the terms \emph{sample} and \emph{population} throughout
earlier chapters, let's now define them formally. A
\emph{population}\index{\emph{population}} is the set of individuals
(\emph{subjects}\index{\emph{population}}) which we're interested in
studying. Subjects may be people, mice, or devices produced in a factory, just
...
then a reasonably big sample taken from this population should also
have approximately 50\% females
...
then our estimates of population features can be trusted with a certain degree of confidence,
and the larger the sample, the more confident we are about the
estimate based on the sample
and the conclusions that we can draw from it.
However, if the sample doesn't faithfully represent the population of interest, then no matter
...
we get a \emph{biased}\index{estimator!\emph{biased}} estimator.
...
\begin{equation}
  ...
  \text{weight} = -194.49 + 5.30 \times \text{height}\,.
\end{equation}

However, because people have different activity levels and different diets, each
person's weight is a little different from the one predicted by Equation (\ref{eq:weightheight}).
We write the weight as a function of height using an equation that includes this 
random noise, by adding
a non-deterministic (\emph{stochastic}) term, denoted by `error':
\begin{equation}
  \label{eq:weightheight-stochastic}
  \text{weight} = -194.49 + 5.30 \times \text{height} + \text{error},
\end{equation}
where, in our simulation, the error has a normal distribution with mean zero and standard deviation
23.26. Models of this form---a linear predictor plus a stochastic error term---are the foundation of
linear regression, which we cover in detail in the next chapter. We simulate such a population with $N=500$ people and plot their weights
against heights using the following code. The result is given in
Figure~\ref{fig:ch7:representativesample} (a), where, in addition to the 
...
Figure~\ref{fig:ch7:representativesample} (b) using the dark triangles. The \code{sample()} function
...
Now, let's see what happens when the sample is not representative. Suppose our
selection depends on weight---for example, selecting the 50 heaviest individuals
in the population. The following code takes this biased sample and plots it in
Figure~\ref{fig:ch7:representativesample} (c).
In the code, \wingding{1} uses \code{order()} to find the indices of the
$n=50$ individuals with the largest weights.
What's important is that the selection of an individual into the sample is
\emph{not random}. When the selection of a subject into the sample
depends on their height, weight, or both, the sample may no longer be representative of the
population, and estimates of population features based on it can be biased.
\showChunk{R}{Code/datacollect-goodbadsample.R}{BIASgbs3}

In Figure~\ref{fig:ch7:representativesample} (c) you can see that the relationship
...
\begin{figure}
    \begin{subfigure}{0.32\textwidth}
      \includegraphics[width=\textwidth]{images/chapter_7/population.pdf}
      \caption{Population}
    \end{subfigure}
  \begin{subfigure}{0.32\textwidth}
      \includegraphics[width=\textwidth]{images/chapter_7/representSample.pdf}
      \caption{Representative sample}
  \end{subfigure}
  \begin{subfigure}{0.32\textwidth}
      \includegraphics[width=\textwidth]{images/chapter_7/biasedSample.pdf}
      \caption{Biased sample}
  \end{subfigure}
  \caption{An illustration of representative and non-representative samples}
  \label{fig:ch7:representativesample}
\end{figure}

To see the effect of a biased sample, let's look at the standard deviation of
weight in the population, and in the two samples. We can use the following code:
\showChunk{R}{Code/datacollect-goodbadsample.R}{BIASgbs4}
The standard deviation in the population is called \code{sd0} in the code and
it's equal to \inlnR{```cat(sd0)```}[sd0]. In the first (simple random) sample
the standard deviation is called \code{sd1} and is equal to
\inlnR{```cat(sd1)```}[sd1], which is very close to the actual standard
deviation in the population (the difference is \inlnR{```cat(sd1-sd0)```}
[diff01]), but in the second (biased) sample, the standard deviation of height
is much smaller: \code{sd2} is \inlnR{```cat(sd2)```}[sd2].
This illustrates the core danger of a biased sample: it doesn't just shift the
mean---it distorts the entire distribution, making the population appear far more
homogeneous than it actually is. Any conclusion drawn from such a sample about
the spread or diversity of the population would be seriously misleading.

\subsection{A Representative Sample Is More Important Than a Larger Sample}\label{representative-size}
...
There were two major flaws in the way Literary Digest collected the sample: 1) The
...
\subsection{Controlled Experiments versus Observational Studies}

Understanding how data is collected is crucial for interpreting the resulting
analyses and for drawing valid conclusions. The two primary approaches in data
...
A typical example of a controlled experiment is a \emph{randomized
  controlled trial} (RCT)\index{\emph{randomized controlled
    trial}}. RCTs are often considered the gold standard for studying
cause and effect. Researchers plan the experiment in advance and
decide what data to collect. RCTs are common in medical research,
where researchers may want to measure how well a new treatment, such
as a new drug, changes an outcome, such as blood pressure.


Many factors besides the treatment may affect the outcome. For
example, age, sex, and body mass index (BMI) may all be related to
blood pressure. In an RCT, researchers randomly assign each
participant to a treatment group. Random assignment tends to produce
comparable groups, so differences in these other factors are less
likely to explain differences in the outcome. The groups will not be
identical, but random assignment makes them comparable on average.


For example, suppose researchers compare two new drugs with an existing
drug. The trial has three treatment groups. If the groups are large
enough, they will
usually have similar age distributions, proportions of females, BMI
distributions, and distributions of other characteristics. The groups
will not be exactly alike, but random assignment helps prevent
systematic differences among them.


This makes comparisons among the three treatments more reliable. If
the groups are similar except for the treatments they receive,
differences in blood pressure are less likely to be explained by age,
sex, BMI, or other characteristics. We can therefore use the
comparison to estimate whether the treatments caused the differences
in blood pressure. Random assignment does not eliminate chance
differences among the groups, but it makes systematic differences less
likely.


The \emph{control} in a randomized controlled trial (RCT)
\index{\emph{randomized controlled trial!control}} is the treatment group
that serves as the baseline for comparison. The control group may receive an
existing treatment, a placebo, or no treatment. In our example, the control
group receives the existing drug. We compare the new drugs with this existing
treatment to see whether they control blood pressure more effectively.

As another example, suppose we want to compare several workout programs. We
could randomly assign participants to different groups and use a ``no
training'' group as the control group. We could then compare changes in body
weight or endurance across the groups. These comparisons would help us
determine whether any of the workout programs works better than no
training.


\jy{Randomization does not require researchers to balance every factor
  themselves, and an unmeasured factor does not automatically prevent
  an RCT from supporting causal conclusions.}
\why{not clear about the point here.}
...
For example, if we want to check the effect of smoking on the risk of getting lung
cancer,
...
the observed differences between the groups are due to the treatments, or to the
missing factors!

\subsubsection{Observational Studies}

Most of the time, the available data is not from RCTs, but from
\emph{observational studies} \index{\emph{observational studies}}. These are typically much easier to collect, but they don't
...
us to establish a cause-and-effect relationship. But, as we said, most of the data
...
\section{Types of Data Problems}\label{types-data-problems}
...
are led to wrong (potentially disastrous) conclusions. 
We will demonstrate some of these data problems in this chapter.

\subsection{Berkson's Bias\index{bias!Berkson's bias}}\label{berkson-bias}
...
Are they? Similarly, you may have seen articles that focus on ``popular books whose 
...
Where do these false impressions come from? The answer is that 
they usually result from a type of \emph{selection bias}, where the data
is not collected at random and some items have a higher or lower probability
...
\begin{example}[Stamps display\index{bias!Berkson's bias!Stamps display}]
  ...
  observe, will they conclude that a rare stamp is more likely to be pretty?
...
The reason for their incorrect conclusion is that the 370 stamps are not a
...
As shown in the Venn diagram in Figure~\ref{fig:ch7:stamps}, the area of the red
...
\caption{Venn diagram for the stamps example. The rectangle represents all the
  ...
  each region.}
  \label{fig:ch7:stamps}
\end{figure}
\end{example}

\begin{example}[Popular books bad movies\index{bias!Berkson's bias!popular books bad movies}]
Now let's simulate some data to further illustrate the problem with ``popular
books whose movie adaptations were not-so-good'' example.

\showChunk{R}{Code/datacollect-berkson.R}{berkson-setup}
\showChunk{R}{Code/datacollect-berkson.R}{berkson-plot-all}
\showChunk{R}{Code/datacollect-berkson.R}{berkson-plot-selected}
\showChunk{R}{Code/datacollect-berkson.R}{berkson-output}
\runR{Code/datacollect-berkson.R}{datacollect-berkson} %

In the above code, the quality scores of books and movies are drawn
independently from a standard normal distribution (\wingding{1} and
\wingding{2}). Because they are generated independently, there is no inherent
relationship between how good a book is and how good its movie adaptation is in
the underlying population.  Line \wingding{3} checks if a book is good (better
than 90\% of all books) and line \wingding{4} checks if a movie is good (better
than 90\% of all movies). A component of the variable \code{good} in line
\wingding{5} is \code{TRUE} if the book, the movie, or both are good. Running
the above code should give the two figures in Figure~\ref{fig:ch7:berkson} and
the following output.
\includeOutput{datacollect-berkson} %

We see from the output and Figure~\ref{fig:ch7:berkson} (a) that the correlation between
the goodness of books and the goodness of movies is very close to zero---there
is no linear association between them. However, if we limit our data to include
...
Figure~\ref{fig:ch7:berkson} (b).


\begin{figure}[H]
  \begin{subfigure}{0.485\textwidth}
    \includegraphics[width=\textwidth]{images/chapter_7/berkson1.pdf}
    \caption{No linear association between goodness of movies and goodness of
      books}
  \end{subfigure}
  \begin{subfigure}{0.485\textwidth}
    \includegraphics[width=\textwidth]{images/chapter_7/berkson2.pdf}
    \caption{Negative linear association for top 10 percent books and/or movies}
  \end{subfigure}
  \caption{Goodness of movies versus goodness of books}
  \label{fig:ch7:berkson}
\end{figure}

People typically only recommend good books or good movies to other
people, so what we see are mostly those with the best reviews. If a book is
bad and the movie based on it is terrible, it's very unlikely you'll ever have
...
The `blank square' in the bottom-left corner of Figure~\ref{fig:ch7:berkson} (b), where we see no
points at all, is our \emph{dark data}\index{bias!Berkson's bias!dark data} in this example, and is the source of
...
Berkson's bias is related to the idea of \emph{conditional probability}\index{\emph{conditional probability}},
...
\subsection{Survival Bias\index{bias!Survival Bias}}\label{survival-bias}

Survival bias arises when observations are limited to subjects or objects that
have ``survived'' or successfully completed a particular process, while those
that did not are overlooked. For instance, when studying tech startups, prominent success stories like
Facebook dominate the spotlight, overshadowing the vast majority of ventures
that failed and quietly disappeared. Ignoring survivor bias can result in misleading
...
\begin{example}[WWII Aircraft Damage\index{bias!Survival Bias!hit aircraft}]
...
Figure~\ref{fig:ch7:survival-bias}. Which areas should the army add armor to
increase the survivorship of the planes?

A first look might suggest adding armor where the returning planes have the
most bullet holes. But this sample only includes planes that made it back. If a
hit to the engine or cockpit brings a plane down, that plane never appears in
the data. So the empty areas in the plot are not safe areas. They are the areas
where damage was often fatal. The right lesson is to armor the parts that are
missing from the returning planes, because those are the parts that probably
need the most protection.
The striking thing is that the places with few bullet holes are the ones we
should worry about most. They are missing precisely because hits there often
kept the plane from returning.
\end{example}
...
\begin{figure}[H]
  ...
  \label{fig:ch7:survival-bias}
\end{figure}

\subsection{Omitted Variables}\label{omitted-vars}
Sometimes we just don't know that we're missing critical data. It can be
in the form of the base-rate fallacy (Chapter~\ref{ch:probability}, Section~\ref{sec:base-rate}),
where the data we may be ignoring (unless we're careful) is the prevalence of something
in the population, such as blood type or a disease. In other cases, we may be missing data
because of \emph{selection bias}\index{bias!selection bias}, where we inadvertently analyze only
...
We've already seen two such examples in previous chapters. The first appears
in Chapter~\ref{ch:EDA}, Figures~\ref{fig:ch2:sidebysidebp} and
\ref{fig:ch2:scatter}. There, the variables were \code{x} (for example, time) and
\code{g} (for example, whether the A/C is working). We saw that only when we
considered both variables \code{x} and \code{g} were we able to see the real
pattern of \code{y} (say, the temperature).

The second appears in Chapter~\ref{ch:probability}, Section~\ref{sec:simpsons-paradox}, where we
...
As we discussed in Chapter~\ref{ch:EDA}, we should always try to use plots and tables
to visualize and understand the nature of the data. In
Chapter~\ref{ch:regression} about regression
we'll see that in some cases we may get hints for missing variables from the so-called
regression diagnostics plots.
However, a missing variable is not always easy to spot, as illustrated in the
following example. 

\begin{example}[House price near campus]
\runR{Code/datacollect-omitted.R}{datacollect-omitted}
Phil is a realtor in a small college town. His
clients often ask why houses farther from campus seem to cost more than houses
within walking distance. Phil plots the data in Figure~\ref{fig:ch7:omitted},
with squared distance from campus on the x-axis and house price on the y-axis.
He also adds a fitted line, which has a positive slope. At first glance, the
plot seems to say that distance and price move together.

\begin{figure}[H]
  \includegraphics[width=.6\textwidth]{images/chapter_7/omitted.pdf}
  ...
  \label{fig:ch7:omitted}
\end{figure}

Phil's plot suggests that distance matters. But the real story is different.
In this simulation, older houses are closer to campus, and older houses are
also cheaper. Distance is not causing the price change here. It is just
standing in for house age. If Phil ignores age, he may still get a fitted line
with a clear trend, but that trend would point to the wrong explanation.

That is the main risk with observational data. A relationship can look
plausible and still be misleading if a key variable is missing. In this case,
we know the true data-generating process because we created it in the
simulation. In real data, we usually do not have that luxury.
That is why a fitted line alone is not enough. We also need to ask what other
variable might be driving the pattern we see.
Once we add age to the story, the pattern makes sense: older houses are both
closer to campus and cheaper, so distance only looks important when age is left
out.

\showChunk{R}{Code/datacollect-omitted.R}{omitted-setup}
\showChunk{R}{Code/datacollect-omitted.R}{omitted-plot}
\end{example}
...
It lets us use historical data to, for example, decide whether to take 
...
Look at Figure~\ref{fig:ch7:ext1}, which shows the relationship between \code{x}
...
\begin{figure}[H]
  \includegraphics[width=.6\textwidth]{images/chapter_7/extra1.pdf}
...
This is a reasonable guess based on the data we have, but is it true?
...
we call \code{e}.
In \wingding{3}, we generate \code{y} using a quadratic function of \code{x}:
$$y = x - 0.4 x^2 + e\,.$$

\showChunk{R}{Code/datacollect-extrapolate.R}{EXT1}

The dashed rectangle in the left panel of Figure~\ref{fig:ch7:ext2} marks the
region where \code{x} is between 0 and 0.25, which corresponds to the narrower
range visible in Figure~\ref{fig:ch7:ext1}.  The right panel enlarges that
rectangle, showing that the data appear nearly linear there: within this
restricted range, the points follow the line $y=x$ so closely that the plot
gives no hint of the underlying quadratic relationship.  However, beyond 0.25,
the quadratic function bends downward and departs drastically from the linear
prediction. While the line $y=x$ extrapolates all the way to $y=2$ when $x=2$,
the true values reach a peak around $x=1.25$ and then fall to around $0.4$.

\begin{figure}[H]
  \includegraphics[width=\textwidth]{images/chapter_7/extra2.pdf}
  \caption{A quadratic relationship that appears linear in a limited range
  of \code{x} but diverges sharply from linear extrapolation.
  Left: full range with the dashed rectangle marking the region shown in
  Figure~\protect\ref{fig:ch7:ext1} and the line $y=x$ showing the linear extrapolation.
  Right: that rectangle enlarged, showing that the data are nearly indistinguishable
  from a straight line within the original sample range.}
  \label{fig:ch7:ext2}
\end{figure}

The code used to produce Figure~\ref{fig:ch7:ext2} is as follows.
...
It is possible to perform a follow-up experiment and check whether our prediction is
correct. For example, suppose that \code{y} in Figure~\ref{fig:ch7:ext1} represents
the average monthly amount spent on online shopping, measured in thousands of dollars
(so \code{y}~$= 0.25$ means \$250 per month), and \code{x} represents annual income,
measured in units of \$100{,}000 (so \code{x}~$= 0.25$ means \$25{,}000 per year).
In our original sample, which gave us Figure~\ref{fig:ch7:ext1}, we only had
people with annual incomes up to about \$25{,}000 spending up to about \$250 per month,
with an approximate linear relationship between \code{x} and \code{y}.
...
we hear on the radio or some streaming service is transmitted as a combination
...
We start by choosing a small time interval and 
divide the interval $[0, 3]$ into equal parts in \wingding{2}.
In  \wingding{3}, we get the sample (for example, by measuring the temperature,
or the frequency of some sound).
We show a plot of the data in the left panel
of Figure~\ref{fig:ch7:aliasing1}. 

\showChunk{R}{Code/datacollect-aliasing.R}{aliasing1}

We know that the data are supposed to be of the form
$$y(t)=\cos(2\pi ft)\,,$$
for some unknown frequency $f$. So, we use the  code in
\wingding{4} and \wingding{5} to interpolate the data at more dense,
...
The right panel of Figure~\ref{fig:ch7:aliasing1} does fit the observed points
perfectly.

But that does not mean it is the right function.

\begin{figure}[H]
  \includegraphics[width=\textwidth]{images/chapter_7/aliasing1.pdf}
  \caption{Sampling from a periodic function, with a low sampling rate}
  \label{fig:ch7:aliasing1}
\end{figure}
...
As you can see in \wingding{2}-\wingding{3}, we now collect data at a rate that
is 10 times higher than before. It is the same code as before, except that the
time interval is 0.01 instead of 0.1. In \wingding{4} we plot the data from
the first, low-rate sample.
The new function is shown in Figure~\ref{fig:ch7:aliasing2}, and it also fits
perfectly.

\begin{figure}[H]
  \includegraphics[width=0.5\textwidth]{images/chapter_7/aliasing2.pdf}
  \caption{Sampling from a periodic function, with a high sampling rate}
  \label{fig:ch7:aliasing2}
\end{figure}

How can this happen? Which function is correct? The answer is that different
sampling rates can lead us to different fitted functions, and all of them can
match the observed points. This is called \emph{aliasing}.

The lesson is simple: a good fit to the sampled data is not always a good
reconstruction of the true function. If the sampling rate is too low, we can
fit the data perfectly and still miss the real pattern.

Now recall the parameter \code{f} in \wingding{1}. That is the true frequency
we used in the simulation. The lower-rate sample in Figure~\ref{fig:ch7:aliasing1}
only shows two cycles between 0 and 1, while the higher-rate sample in
Figure~\ref{fig:ch7:aliasing2} shows all eight cycles in one time unit. The
higher sampling rate reveals the true function.

So, when we work with periodic data, we need to know something about the
fastest possible oscillation. Temperature data usually change slowly, with one
cycle per year. Audio data can change much faster, so sound must be sampled at
a much higher rate. If we sample too slowly, aliasing can hide the real
pattern.
In other words, sampling rate is part of the model. If we ignore it, we may
fit the data perfectly and still learn the wrong function.

We noted in this chapter that a representative sample is more
important than a large sample, but here we see that in some situations
the sample size also matters. In the examples discussed here,
...
\emph{Confirmation bias}, which we also saw in the omitted-variables example
above, occurs when people search for, interpret, and recall information in ways
that confirm their prior beliefs. It is sometimes called \emph{cherry picking}:
the picker selects only the ripest and most appealing fruit, and the collection
is not representative of all the fruit in the orchard.
...
Even when data is collected in a carefully designed experiment, some bias may occur.
For example, rejection of bad data on arbitrary grounds instead of according to
a previously stated or generally agreed criteria---for instance, discarding
what one perceives as ``outliers'' without valid reasons.
...
sample, or in which group in the sample. Volunteers tend to have intrinsically different characteristics from
...
may produce duplicate selections (for example, the birthday problem in
...
\item Cluster Sampling: Divide the population into clusters (for example, geographic
...
reaching wrong conclusions when data collection is not done right. We introduced
the notion of a representative sample, and demonstrated what can happen when a
sample is non-representative (biased). We saw that even a very large sample can
give badly wrong estimates if it is collected in a biased way. We also examined
the difference between randomized controlled experiments---which can establish
causation---and observational studies, which can only reveal associations. We
then surveyed several specific data-collection hazards: Berkson's bias, survival
bias, omitted variables, extrapolation, and aliasing. Finally, we described the
most common sampling designs and noted when each is appropriate.

When the data collection is done as a planned experiment we can prevent many
biases---but this still requires careful design. When we use observational data
there is often little we can do to prevent bias entirely. We should, however,
always be aware of the possible pitfalls, and be careful not to draw
far-reaching or decisive conclusions when we cannot rule out hidden bias.

In Chapter~\ref{ch:regression} we will build regression models that quantify
relationships between variables. The ideas developed here---representative
sampling, omitted variable bias, and the limits of observational data---will
reappear throughout that chapter as we ask whether a fitted model can be
trusted and what its estimates actually mean.
