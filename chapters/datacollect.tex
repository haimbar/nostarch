...
You can't make bricks without straw. In the previous chapters we built tools---the
law of large numbers, the central limit theorem, and probability theory---that let
...
and the larger the sample, the more confident we are about the estimate
...
A typical example of a controlled experiment is a \emph{randomized controlled
  trial} \index{\emph{randomized controlled
...
research, where we wish to evaluate the effectiveness of a new treatment (for example,
a new drug) on an outcome (for example, blood pressure).
...
In a randomized controlled trial we can get a representative sample of the population,
...
the same distribution of BMI, and so on.

Then, the comparison between the three types of treatments is meaningful,
and we can use it to draw conclusions about the effect of the treatment (drug, in our
example) on the outcome (blood pressure). If, apart from the treatment, the
subpopulations in all groups are identical, then any difference in the outcome between
the groups must be due to the one thing that separates them: the treatment itself.

The \emph{control} \index{\emph{randomized controlled
  trial!control}} part in RCT means that one of the treatment groups is our baseline,
