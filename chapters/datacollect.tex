...
or other thinks are related to one's blood pressure.
...
and we can use it to draw conclusions about the effect of the treatment (e.g.,
drug) on the outcome (e.g., blood pressure). Because, if other than the treatment,
the subpopulations in all groups are identical, then if there is a difference between
the group in the outcome, then it must be due to the one thing that separates the
groups.
...
to gather the sample. Second, it may be impossible to randomize them into groups
...
guarantee proper randomization, and that may lead to biased results. Whenever we
analyze data, we must know how it was collected. Was it through a randomized 
...
doesn't come from a RCT. In these case, we must be careful when we draw conclusions.
...
\why{I will add a Venn diagram here.}
