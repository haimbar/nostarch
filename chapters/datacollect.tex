...
The three functions are shown in Figure~\ref{fig:ch7:ext2}.

\showChunk{R}{Code/datacollect-extrapolate.R}{EXT1}

The dashed rectangle in the left panel of Figure~\ref{fig:ch7:ext2} marks the region
where \code{x} is between 0 and 0.4, which covers the narrower range visible in Figure~\ref{fig:ch7:ext1}.
The right panel enlarges that rectangle, showing that all three functions are
nearly indistinguishable there: this is exactly why Figure~\ref{fig:ch7:ext1} alone
gives no clue which function generated the data.
...
rate at first, but then has a change-point where the trend reverses and it starts to decrease.

\begin{figure}[H]
  \includegraphics[width=\textwidth]{images/chapter_7/extra2.pdf}
  \caption{Three functions that look nearly identical in the limited range of
  Figure~\protect\ref{fig:ch7:ext1} but diverge sharply beyond it.
  Left: full range with the dashed rectangle marking the region shown in
  Figure~\protect\ref{fig:ch7:ext1}. Right: that rectangle enlarged, showing
  all three functions are nearly indistinguishable within the original data range.}
  \label{fig:ch7:ext2}
\end{figure}

The code used to produce  Figure~\ref{fig:ch7:ext2} is as follows. Note that we
