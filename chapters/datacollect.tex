...
people, mice, or devices produced in a factory. 
...
However, because people have different levels of activity and different diets, each
person's weight is a little different than the one predicted by Equation (\ref{eq:weightheight}).
We write the weight using an equation that includes this random `noise', by adding
...
Figure~\ref{fig:representativesample} (b). The \code{sample()} function
...
probability of including the person into the sample. The exact way we 
...
When the probability of drawing a subject into the sample depend on his/her
...
between the height and the weight in the population is a long the line
...
The variance in the population is \code{var0} and it's equal to
\inlnR{```cat(var0)```}[var0]. In the first (simple random) sample the
variance is \inlnR{```cat(var1)```}[var1], which is very close to the
actual variance in the population (the difference is \inlnR{```cat(var1-var0)```} [diff01]),
but in the second (biased) sample, the variance of height is much smaller: 
\inlnR{```cat(var2)```}[var2].
...
In the above code, the goodness of books and movies are independently from a the
