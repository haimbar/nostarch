...
theorem, and statistical inferences---that let
...
likely to explain differences in the outcome.


For example, suppose
researchers compare two new drugs with an existing drug. The trial has
three treatment groups. If the groups are large enough, they will
...
because these are the most-hit region. However, the data was collected from the
aircraft that had survived their missions, and if an aircraft was hit and downed
it would not be included in the data. If there is a bullet hole in the engine
area or in the cockpit, then it would be unlikely that the aircraft could
return. This was actually the reason why the engine and cockpit regions were
almost free of bullet holes. Knowing that the marked dots for bullet holes were
a biased sample and the cause of the bias, we should realize that these regions
...
Phil collected data and plotted the graph in Figure~\ref{fig:ch7:omitted},
...
but the motivation for this is that in reality, college towns often expand around the
campus.)
...
We can see  in the right panel of  Figure~\ref{fig:ch7:aliasing1} that the 
...
As you can see in \wingding{2}-\wingding{3}, we now collect data for
our sample at a rate that is 10 times higher than before (it's the same code
as before, except that the time intervals are 0.01, instead of 0.1). In \wingding{4} 
we plot the data that we got from the first (low sampling rate) data.
The new function is shown in Figure~\ref{fig:ch7:aliasing2}, and it also fits perfectly.
...
\ref{fig:ch7:aliasing1} and \ref{fig:ch7:aliasing2}. Can you tell which one is the correct 
function? The function in Figure~\ref{fig:ch7:aliasing1} completes only two
cycles between 0 and 1, while the one in Figure~\ref{fig:ch7:aliasing2}
shows 8 cycles in one time unit. The higher sampling rate gave us the
true function. The lower sampling rate, although it fits the given data perfectly,
