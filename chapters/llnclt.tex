...
\chapter{The Law of Large Numbers and the Central Limit Theorem:
The Pillars of Statistics}\label{ch:llnclt}}
...
random process, what can we expect to see, and how does our confidence grow as
we collect more observations?
...
We'll demonstrate both theorems through simulation across normal, binomial,
and Poisson distributions. You'll see why the sample mean converges to
the population mean (LLN), why its spread shrinks as $\sigma/\sqrt{n}$
(CLT), and apply these ideas to IQ scores and birth-ratio problems.
...
The points in the Q-Q plot in Figure~\ref{fig:ch4:qqplot1} show the actual quantiles
...
several orders of magnitude so the convergence\index{convergence} is easy to see. We'll generate
a random sample for each of these \lstinline{ns}. In \wingding{2} we initialize a vector
which will hold all the sample means (at first, they're set to zero). The
...
We also see in Figure~\ref{fig:ch4:LLNdemo} that the dispersion of the sample means
...
boy-births we expect to see (\emph{binomial distribution}\index{binomial distribution}), the number of
received text messages (\emph{Poisson distribution}\index{Poisson distribution}), or if we measure IQ
...
actually has a mean. (We'll see an example where this is not the case.)
...
Notice that the distribution in Figure~\ref{fig:ch4:CLTexp} is very skewed, and doesn't
...
another, very different distribution. This time our data comes from a discrete
...
You should get the plot in Figure~\ref{fig:ch4:CLTbinom}. Notice that because we've
...
it be surprising if we observed 14,800 boys?
...
The result is shown in Figure~\ref{fig:ch4:CLTboygirl}.
...
Figure~\ref{fig:ch4:CLTIQfit} shows the result. The red curve shows the true density
...
Figure~\ref{fig:ch4:CLTIQMensa} shows the result. The shaded area covers exactly 2\% of
the area under the curve.

\begin{figure}[H]
\includegraphics[width=0.6\textwidth]{images/chapter_4/IQtail.pdf}
\caption{The distribution of IQs and the acceptance range to the Mensa club}
