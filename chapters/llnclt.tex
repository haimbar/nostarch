...
such question is to take a sample from the population we're interested in, and
choose a mathematical model which we believe represents the whole population.
Then we check if our model appears to reasonable, in the sense that it fits the
...
So, we propose a certain distribution and check if it fits. In the current
...
The points in the Q-Q plot in Figure \ref{qqplot1} shows the actual quantiles
(from the data, \lstinline{samp1}) vs. the theoretical quantiles (those that
would be obtained if we drew from a normal distribution). The points in the
graph lie very close to a straight line, indicating that the sample  was
likely drawn from a normal distribution (here, this is not surprising because
we generated it this way. However, when we get a random sample and we do not
know the true distribution, this plot is useful to check whether our
...
gets smaller and the sample size gets larger, but this is the result of the
\emph{Central Limit Theorem}, which will be discussed later in this chapter.

\subsection{Example \#2 -- the Roulette Wheel}
Here, the outcome is binary -- we either win or lose. The probability of
...
gets closer to the true mean of the distribution, which is $p=18/37$, which is
...
Informally, the law of large number states that the results of an experiment
...
It's important to emphasize that the LLN does not imply that the results of
future experiments will balance out what already happened! If we bet 100 times
at the roulette wheel and lost each time, this does not mean that we have a
...
(both follow a normal distribution), with the same sample size. In the for loop
...
true difference between the groups (10), but the ratio, $r_{AB}$, does not
...
doesn't have a theoretical mean, so the LLN does not apply to the ratios
...
distributions may lead to a distribution to which the LLN does not apply. Other
...
a fixed value (the true population mean).  It does not say anything, however,
about how close the sample estimate is to the true value. In other words, it
says nothing about our level of (un)certainty. 

The central limit theorem (CLT) says that, in most situations, if we could get
...
sample has to be i.i.d., and the distribution must have a finite variance. 
...
look at all like a normal distribution. Try running \lstinline{mean(samp)}. you
should get \inlnR{```cat(mean(samp))```}[llnmeansampexp], which is approximately
...
When $n=30000$ the red line is well outside the range of the simulated sample
means.
...
multiple samples}.
Not only that, but regardless of the distribution of the data, the sample mean
...
confident intervals for estimates. For example, we will be able to answer
